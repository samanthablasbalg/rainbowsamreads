"""remove legacy progress log ranges

Revision ID: 91b7c3e5a024
Revises: 6a1d8e4c9f20
Create Date: 2026-08-31 02:15:00.000000
...
revision: str = "91b7c3e5a024"
down_revision: str | Sequence[str] | None = "6a1d8e4c9f20"
...
def upgrade() -> None:
    ...
    op.drop_column("progress_logs", "page_start")
    op.drop_column("progress_logs", "page_end")
    op.drop_column("progress_logs", "minute_start")
    op.drop_column("progress_logs", "minute_end")


def downgrade() -> None:
    op.add_column(
        "progress_logs", sa.Column("page_start", sa.Integer(), nullable=True)
    )
    op.add_column("progress_logs", sa.Column("page_end", sa.Integer(), nullable=True))
    op.add_column(
        "progress_logs", sa.Column("minute_start", sa.Integer(), nullable=True)
    )
    op.add_column(
        "progress_logs", sa.Column("minute_end", sa.Integer(), nullable=True)
    )
    op.execute(
        """
        UPDATE progress_logs
        SET page_start = CASE WHEN unit = 'pages' THEN start END,
            page_end = CASE WHEN unit = 'pages' THEN "end" END,
            minute_start = CASE WHEN unit = 'minutes' THEN start END,
            minute_end = CASE WHEN unit = 'minutes' THEN "end" END
        """
    )
    op.execute(
        """
