...
`samanthablasbalg`, repo `samanthablasbalg/reading-tracker`). Every issue MUST live on that board,
...
2. **Find the parent epic.** List current epics and match:
   `gh issue list --repo samanthablasbalg/reading-tracker --label epic --limit 50` If none fits
...
8. **Verify** (step 5): re-read the issue, confirm parent + board membership + column + milestone,
   and report what you set. Do not assume the writes landed.
...
Progress `1e2e38c0` · Done `b578580f`. (Stub → Backlog `d8528043`. Refined → To Develop `36ff1833`.)
...
Confirm `parent:`, `projects:` (with the column), and `milestone:` are what you intended. Report the
result.
