      ...
      <DrawerPrimitive.VirtualKeyboardProvider>
        ...
        >
          ...
          >
            ...
            {/* Not `Drawer.Content`: that part is reserved for `DrawerBody` below, the one
                actually scrollable element. Base UI's keyboard-scroll targeting walks up
                from the focused field looking for a scrollable ancestor, so header and
                footer must sit outside it -- otherwise they'd scroll away with the body.
...
function DrawerFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        // The base 1rem matches the popup content's own pt-4/px-4 -- the wrapper carries
        // no pb-4 of its own, so the footer owns all of its bottom spacing. The fallback
        // in the var() is required: VirtualKeyboardProvider only sets it while a field is
        // focused above the software keyboard, so it's unset (not 0px) the rest of the time.
        'mt-auto flex shrink-0 flex-col gap-2 pb-[calc(var(--drawer-keyboard-inset,0px)+1rem)]',
        className
      )}
      {...props}
    />
